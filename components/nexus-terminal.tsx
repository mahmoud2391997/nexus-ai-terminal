    ...
    <main className="min-h-screen bg-background text-foreground selection:bg-primary selection:text-primary-foreground">
      ...
      <div className="mx-auto grid max-w-[1500px] lg:grid-cols-[220px_minmax(0,1fr)_280px]">
        ...
        <section id="terminal" className="min-w-0 border-border lg:border-r">
          ...
          <div className="sticky bottom-0 border-t border-border bg-card/95 p-4 md:p-6"><div className="mx-auto max-w-3xl"><div className="relative rounded-lg border border-input bg-background shadow-sm focus-within:ring-2 focus-within:ring-ring"><textarea value={command} onChange={(event) => setCommand(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing && event.keyCode !== 229) { event.preventDefault(); submitCommand() } }} placeholder="Type a command for Nexus..." className="min-h-14 w-full resize-none bg-transparent px-4 pb-12 pt-3 text-sm outline-none placeholder:text-muted-foreground" aria-label="Command input" /><div className="absolute bottom-2 left-3 flex items-center gap-1 text-[11px] text-muted-foreground"><kbd className="rounded border border-border px-1.5 py-0.5 font-mono">↵</kbd> send <span className="mx-1">·</span><kbd className="rounded border border-border px-1.5 py-0.5 font-mono">⇧ ↵</kbd> new line</div><div className="absolute bottom-2 right-2 flex items-center gap-1"><button onClick={() => setIsListening((value) => !value)} className={`rounded-md p-2 ${isListening ? 'bg-destructive/10 text-destructive' : 'text-muted-foreground hover:bg-muted'}`} aria-label={isListening ? 'Stop recording' : 'Start voice input'}>{isListening ? <X className="size-4" /> : <Mic className="size-4" />}</button><button onClick={submitCommand} className="rounded-md bg-primary p-2 text-primary-foreground hover:opacity-90" aria-label="Send command"><ArrowUp className="size-4" /></button></div></div>{isListening && <div className="mt-2 flex items-center gap-2 font-mono text-xs text-destructive"><span className="size-2 animate-pulse rounded-full bg-destructive" /> Listening · release to stop</div>}</div></div>
        </section>
